...
class View( object ):
	...
	def __init__(self, dbpath):
		...
		appuifw.app.title = u"Tickets View"
		db.open(self.dbpath)
		# self.list_fuel = []
	...
	def _initialize_cabins(self):
		appuifw.app.menu = [(u"Select", self._select_tickets), (u"Delete", self.__delete_field), (u"Back", self.back)]
		self.__create_list()
		try:
			self.list_box_tickets = appuifw.Listbox(map(lambda x:x[1], self.list_buy))
		except:
			self.list_empty = []
			self.list_empty.append(u"< Empty >")
			self.list_box_tickets = appuifw.Listbox(map(lambda x:x, self.list_empty))
		self.list_box_tickets.bind(key_codes.EKeySelect, self._select_tickets)
		self.list_box_tickets.bind(key_codes.EKeyBackspace, self.__delete_field)
		appuifw.app.body = self.list_box_tickets
		appuifw.app.exit_key_handler = self.back

	def _select_tickets(self):
		if len(self.list_tickets) > 0:
			id = self.list_box_tickets.current()
			self.__get_info(self.list_tickets[id][0])
			self.__show_form(self.info_selection)

	def __show_form(self, lista):
		old_title = appuifw.app.title
		appuifw.app.title = u"ID: %s Tickets" % lista[0]
		self._iFields = [( u"Date", "date", lista[1]),
						 ( u"Cabina", "text", lista[2]),
						 ( u"Strumento", "text", lista[3]),
						 ( u"Note", "text", lista[4] )]
	...
	def __get_info(self, id):
		import globalui
		sql_string = u"SELECT * FROM tickets WHERE id=%d" % int(id)
	...
	def __create_list(self):
		self.list_tickets = []
		sql_string = u"SELECT * FROM tickets"
		try: 
			dbv.prepare(db, sql_string)
		except:
			db.open(self.dbpath)
		...
		for i in range(1,dbv.count_line()+1):
			...
			self.list_tickets.append((result[0], unicode("[%s] %s" % (result[0], strftime("%d/%m/%Y", time.localtime(result[1]))))))
			dbv.next_line()
		db.close()
